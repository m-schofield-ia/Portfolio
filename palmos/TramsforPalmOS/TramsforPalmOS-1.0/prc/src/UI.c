...
UIInit(void)
{
	...
	__ui.oneHandedEnabled=FrmGlueNavIsSupported();
	return &__ui;
}
 ...
 * UIObjectFocus
 *
 * Bring this object to Focus.
 *
 *   -> id		Object id in current form.
 */
void
UIObjectFocus(UInt16 id)
{
 ...
 *  -> fId		Field ID.
 *  -> sId		ScrollBar ID.
 *  -> onlyOne		Only one field/scrollbar on form (disable checks).
 *
 * Returns true if handled, false otherwise.
 */
Boolean
UIFieldScrollBarKeyHandler(EventType *ev, UInt16 fId, UInt16 sId, Boolean onlyOne)
{
	if (!onlyOne) {
		Int16 focusIndex;
		UInt16 focusId;
 ...
 */
Boolean
UIListKeyHandler(EventType *ev, UInt16 id)
{
	Int16 focusIndex;
	...
	if (FrmGetObjectId(currentForm, focusIndex)==id) {
		ListType *lst=UIObjectGet(id);
		Int16 page;
