 ...
 *	Blowfish.
 *
 *	Implementation of the public available blowfish encryption engine.
 *	This implementation is not particular fast ... but short :)
 */
#include "public.h"

/* protos */
//static void bfenc(blowfishkey *, unsigned long *, unsigned long *);
//static void bfdec(blowfishkey *, unsigned long *, unsigned long *);
//unsigned long *sb1, *sb2, *sb3, *sb4;

/*******************************************************************************
 *
 *	Tables.
 *
 *	These blowfish tables were created by Mike Schaudies.	The tables
 *	were found at:
 *
 ...
 *		=============================================================
 */
static const unsigned long sbox1[] = {
...
static const unsigned long sbox2[] = {
...
static const unsigned long sbox3[] = {
...
static const unsigned long sbox4[] = {
...
static const unsigned long parray[] = {
...
/* macros */
#define function(x) \
	(((sb1[(x>>24)&0xff]+sb2[(x>>16)&0xff])^(sb3[(x>>8)&0xff]))+(sb4[x&0xff]))

/*
static unsigned long function(unsigned long);

static unsigned long function(unsigned long x)
{
	unsigned long y;
 ...
 *
 *	Input:
 *		ks - where to store keyset.
 *		key - new key.
 *		len - new key length.
 */
void
blowfishmakekey(blowfishkey *ks,
                unsigned char *key,
                unsigned int len)
{
	unsigned long dl=0, dr=0, d;
	int h=0, i, j;

	MemMove(ks->sbox1, sbox1, sizeof(sbox1));
	MemMove(ks->sbox2, sbox2, sizeof(sbox2));
	MemMove(ks->sbox3, sbox3, sizeof(sbox3));
	MemMove(ks->sbox4, sbox4, sizeof(sbox4));
 ...
 *	Input:
 *              ks - key set.
 *		l1 - ptr to longword 1 to encrypt.
 *		l2 - ptr to longword 2 to encrypt.
 */
void
bfenc(blowfishkey *ks,
      unsigned long *l1,
      unsigned long *l2)
{
	unsigned long xl=*l1, xr=*l2;
	unsigned long *pa=ks->parray;
	unsigned long *sb1=ks->sbox1, *sb2=ks->sbox2, *sb3=ks->sbox3, *sb4=ks->sbox4;
 ...
 *	Input:
 *              ks - key set.
 *		l1 - ptr to longword 1 to decrypt.
 *		l2 - ptr to longword 2 to decrypt.
 */
void
bfdec(blowfishkey *ks,
      unsigned long *l1,
      unsigned long *l2)
{
	unsigned long xl=*l1, xr=*l2;
...
*	blowfishencrypt
*
*	Public interface to blowfish encrypt.
*
*	Input:
*		ks - key set.
*		src - source.
*/
void
blowfishencrypt(blowfishkey *ks,
                unsigned char *src)
{
...
*	blowfishdecrypt
*
*	Public interface to blowfish decrypt.
*
*	Input:
*		ks - key set.
*		src - source.
*/
void
blowfishdecrypt(blowfishkey *ks,
                unsigned char *src)
{
	unsigned long *p=(unsigned long *)src;
	unsigned long l1, l2;
