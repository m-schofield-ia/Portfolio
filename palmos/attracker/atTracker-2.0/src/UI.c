 ...
 */
void
UITableInit(DB *db, Table *table, UInt16 id, UInt16 scrBarId, TableDrawItemFuncType *dc, TableStyle *style)
{
 ...
 */
void
UIPopupSet(DB *dbo, UInt16 idx, Char *dst, UInt16 id)
{
	CategoryGetName(dbo->db, idx, dst);
...
**	UIGadgetHandler
**
**	Invert and handle Gadgets.
**
**	 -> ev		Event.
**	 -> id		Gadget ID.
**
**	Returns true if pen was lifted when over gadget, false otherwise.
*/
Boolean
UIGadgetHandler(EventType *ev, UInt16 id)
{
	Boolean wasInBounds=true, nowInBounds, penDown;
	RectangleType r;
	Int16 x, y;

	FrmGetObjectBounds(currentForm, FrmGetObjectIndex(currentForm, id), &r);
