...
UIInit(void)
{
	...
	__ui.navSupported=FrmGlueNavIsSupported();
 ...
 *  -> items		No. of items in list.
 *  -> selected		Selected item.
 */
void
UIListInit(UInt16 id, void (*drawFunc)(Int16, RectangleType *, Char **), Int16 items, Int16 selected)
{
	ListType *lst=UIObjectGet(id);

	LstSetDrawFunction(lst, drawFunc);
	LstSetListChoices(lst, NULL, items);
	LstSetSelection(lst, selected);

	if (selected==noListSelection)
		LstMakeItemVisible(lst, 0);
	else
		LstMakeItemVisible(lst, selected);
}

/*
 * UIListKeyHandler
 *
 * Handles events for this list.
 *
 *  -> ev		Event.
 *  -> id		List ID.
 *  -> fiveWay		Handle special 5 way (Treo) rocker.
 *
 * Returns false.
 */
Boolean
UIListKeyHandler(EventType *ev, UInt16 id, UInt16 fiveWay)
{
	ListType *lst=UIObjectGet(id);
	Int16 page;

	page=LstGetVisibleItems(lst)-1;

	if (ev->data.keyDown.chr==pageUpChr) {
		LstScrollList(lst, winUp, page);
		return true;
	} else if (ev->data.keyDown.chr==pageDownChr) {
		LstScrollList(lst, winDown, page);
		return true;
	}

	if ((__ui.navSupported) && (fiveWay)) {
		UInt16 navId;
		Int16 eInfo;

		if (FrmGlueNavGetFocusRingInfo(currentForm, &navId, &eInfo, NULL, NULL)==errNone) {
			if ((navId==id) && (eInfo==noListSelection)) {
				if (ev->data.keyDown.chr==vchrRockerUp) {
					LstScrollList(lst, winUp, page);
					return true;
				} else if (ev->data.keyDown.chr==vchrRockerDown) {
					LstScrollList(lst, winDown, page);
					return true;
				}
			}
...
/**
 * Update the focus information for the list (and redraw).
 *
 * @param id Object id.
 * @param idx Index in list (or noListSelection).
 */
void
UIListUpdateFocus(UInt16 id, Int16 idx)
{
	ListType *lst=UIObjectGet(id);

	LstSetTopItem(lst, idx);
	LstSetSelection(lst, idx);
	LstDrawList(lst);
	FrmSetFocus(currentForm, FrmGetObjectIndex(currentForm, id));

	if (__ui.navSupported)
		FrmGlueNavObjectTakeFocus(currentForm, id);
}
