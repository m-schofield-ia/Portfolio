...
/* macros */
#define function(x) (((sb1[(x>>24)&0xff]+sb2[(x>>16)&0xff])^(sb3[(x>>8)&0xff]))+(sb4[x&0xff]))
#define blowalgol(x) xl=xl^(pa[x]); xr=function(xl)^xr;
#define blowalgor(x) xr=xr^(pa[x]); xl=function(xr)^xl;

/*
 * BlowfishMakeKey
 *
 * Generate new Blowfish key set.
 *
 * <-  ks		Where to store key set.
 *  -> key		"Key" (= password).
 *  -> len		Key length.
 */
void
BlowfishMakeKey(BlowfishKey *ks, UInt8 *key, UInt32 len)
{
	UInt32 h=0, i, j, d;
	UInt8 bits[8];
	UInt32 *bp1, *bp2;
 ...
 * BlowfishEncrypt
 *
 * Encrypt 64 bits. Encryption are done in place.
 *
 *  -> ks		Key set.
 * <-> src		Source data.
 */
void
BlowfishEncrypt(BlowfishKey *ks, UInt8 *src)
{
	UInt32 *p=(UInt32 *)src;
	UInt32 *sb1=ks->sbox1, *sb2=ks->sbox2, *sb3=ks->sbox3, *sb4=ks->sbox4, xl, xr;
	UInt32 *pa=ks->parray;
 ...
 * BlowfishDecrypt
 *
 * Decrypt 64 bits. Decryption are done in place.
 *
 *  -> ks		Key set.
 * <-> src		Source data.
 */
void
BlowfishDecrypt(BlowfishKey *ks, UInt8 *src)
{
	UInt32 *p=(UInt32 *)src;
	UInt32 *sb1=ks->sbox1, *sb2=ks->sbox2, *sb3=ks->sbox3, *sb4=ks->sbox4, xl, xr;
	UInt32 *pa=ks->parray;
