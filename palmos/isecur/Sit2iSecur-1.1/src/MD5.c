...
/* protos */
#define RP(rn) static void rn(UInt32 *a, UInt32 b, UInt32 c, UInt32 d, UInt32 x, UInt16 s, UInt32 ac)
RP(round1);
RP(round2);
RP(round3);
RP(round4);
 ...
 *
 * <-  dst		Where to store MD5 sum.
 *  -> src		Source data.
 */
void
MD5Sum(UInt32 *dst, UInt8 *src)
{
	UInt32 a=0x67452301, b=0xefcdab89, c=0x98badcfe, d=0x10325476, i, j;
	UInt32 x[16];
	UInt8 buffer[64];
	...
	for (j=0; j<64; j+=4) {
		x[i]=(UInt32)buffer[j];
		x[i]|=((UInt32)buffer[j+1])<<8;
		x[i]|=((UInt32)buffer[j+2])<<16;
		x[i]|=((UInt32)buffer[j+3])<<24;
		i++;
	}
