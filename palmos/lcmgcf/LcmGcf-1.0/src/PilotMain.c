...
#ifdef DEBUG
#if SYS==0
	gHostFile=HostFOpen("/tmp/LcmGcf.log", "a+");
#else
	gHostFile=HostFOpen("c:\\tmp\\LcmGcf.log", "a+");
#endif
	ErrFatalDisplayIf(gHostFile==NULL, "(Start) DOpen failed");
	DPrint("======== LcmGcf Started ====\n");
#endif
	...
	for (EVER) {
		...
		if (session && session->timedSecs>0 && inGame) {
			...
			if (endQuiz) {
				...
				CtlSetEnabled(UIObjectGet(cGameStop), 0);
				CtlSetEnabled(UIObjectGet(cGameAnswer), 0);

				for (idx=cGameBut0; idx<=cGameButOK; idx++)
					CtlSetEnabled(UIObjectGet(idx), 0);
